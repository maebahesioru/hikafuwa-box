...
const SLUG = "hikafuwa-box-pro";
const PRICE = 600;
const STORAGE_KEY = "hkm_pro_hikafuwa_box";
...
export function HkmProBanner() {
  ...
  useEffect(() => {
    // Check token from URL (callback from HKM checkout)
  ...
  const checkoutUrl = `${HKM_API}/checkout?item=${SLUG}&callback=${encodeURIComponent(callbackUrl)}`;
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 rounded-xl border border-yellow-500/30 bg-black/90 p-4 shadow-2xl max-w-xs">
      ...
      <p className="text-xs text-gray-300 mb-3">月額{PRICE} HKMで広告非表示・Pro機能解放</p>
      <a href={checkoutUrl} className="block w-full rounded-lg bg-yellow-400 py-2 text-center text-xs font-bold text-black hover:bg-yellow-300">
        {PRICE} HKM/月で購入
      </a>
    </div>
  );
